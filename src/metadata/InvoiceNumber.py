from Utils import *
def EFNInvoiceNumber(textBlob, possibleKey,SEARCH_STRATEGY):
    """Gets the possible invoice number from the list of textBox
...
def IPInvoiceNumber(possibleKey):
    """Gets the possible invoice number from the list of possible key.
...
def qualifiesInvoiceNumber(result):
    ...
    ----------
    result : str
        string value for the extracted value for invoice number key
...
def isInvoiceNumberInNear(p):
    ...
    -------
    str
        final string value for extracted number
    """
